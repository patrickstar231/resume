...
function Project({ route }) {
  ...
  return <main id="main" className="project-page container" tabIndex={-1}>
    ...
    <div className="case-heading"><p className="project-category">{copy.category}</p><h1>{copy.title}</h1><p className="case-intro">{copy.intro}</p></div>
    <div className="case-body">
      <aside className="case-facts" aria-label={locale === 'zh' ? '项目资料' : 'Project information'}><dl>
        <div><dt>{t.role}</dt><dd>{copy.role}</dd></div><div><dt>{t.brand}</dt><dd>{project.brand}</dd></div>
...
function Resume({ locale }) {
  ...
  return <main id="main" className="resume-page container" tabIndex={-1}>
    ...
    <div className="resume-heading"><h1>{locale === 'zh' ? '潘宇龙' : 'Patrick Pan'}</h1>{locale === 'zh' && <p className="resume-english-name">Patrick Pan</p>}<p className="resume-title">{profile.title[locale]}</p><a className="text-link" href={`mailto:${profile.email}`}>{profile.email}</a></div>
    ...
    <section className="resume-section"><h2>{t.education}</h2><div className="education-grid">{education.map(item => <article key={item.dates}><p className="experience-date">{item.dates}</p><h3>{item.school[locale]}</h3><p>{item.course[locale]}</p></article>)}</div></section>
