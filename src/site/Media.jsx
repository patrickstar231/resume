...
export function Photo({ name, locale, alt, className = '', motionKind, priority = false }) {
  ...
  const widths = name === 'snow' ? [480, 640, 768] : [640, 960, 1440];
  const sourceSet = (base, extension) => widths.map(width => `/images/${base}-${width}.${extension} ${width}w`).join(', ');
  const ratio = name === 'desk' ? [4, 5] : name === 'snow' ? [3, 4] : [4, 3];
  const sizes = name === 'snow' ? '(max-width: 767px) 80vw, 28vw' : '(max-width: 767px) calc(100vw - 40px), 50vw';
