...
test('desktop content hydrates, switches themes, and keeps the first-screen actions visible', async ({ page }) => {
  const errors=[]; page.on('pageerror', error => errors.push(error.message));
  await page.goto('/');
  await expect(page.locator('html')).toHaveAttribute('data-enhanced','true');
  const button = page.getByRole('link',{ name:'View projects',exact:true }).first();
  ...
  await expect.poll(() => page.locator('#life img').evaluateAll(images => images.every(img => img.complete && img.naturalWidth > 0))).toBe(true);
...
test('mobile layouts fit and the menu traps focus and closes with Escape', async ({ page }) => {
  for (const width of [320,390,768]) {
    ...
    const button=page.getByRole('link',{ name:'查看项目',exact:true }).first(); const box=await button.boundingBox(); expect(box.y+box.height).toBeLessThan(844);
  }
...
test('case and resume language links preserve the page, history works, and missing paths stay 404', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/zh\/projects\/porsche-992\/$/);
  await expect(page.getByRole('heading',{ level:1 })).toHaveText('保时捷 992 新车发布项目');
  ...
  const response=await page.goto('/zh/missing-page/'); expect(response.status()).toBe(404);
...
test('without JavaScript, content, native details, language navigation and contact remain usable', async ({ browser }) => {
  ...
  const page=await context.newPage(); await page.goto('http://127.0.0.1:4173/zh/');
